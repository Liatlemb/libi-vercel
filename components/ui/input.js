...
export default function LibiChat() {
  ...
  return (
    <div style={{ maxWidth: "600px", margin: "auto", padding: "20px", fontFamily: "Arial", direction: "rtl" }}>
      <div style={{ border: "1px solid #ccc", borderRadius: "8px", padding: "16px", backgroundColor: "#fffef0" }}>
        {messages.map((m, i) => (
          <div key={i} style={{
            textAlign: m.role === 'user' ? 'right' : 'left',
            margin: "8px 0",
            whiteSpace: "pre-wrap",
            color: m.role === 'assistant' ? '#444' : '#222'
          }}>
            <strong>{m.role === 'user' ? 'את' : m.role === 'assistant' ? 'ליבי' : ''}</strong>: {m.content}
          </div>
        ))}
